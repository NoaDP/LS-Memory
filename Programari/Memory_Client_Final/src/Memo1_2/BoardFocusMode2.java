...
public class BoardFocusMode2 extends BoardFocusMode{

	/**
	 * La classe BoardFocusMode2 és una classe que hereta de la classe BoardFocusMode i és l'encarregada de crear i 
	 * modificar els elements que componen el tauler de joc del mode "Focus" en dificultat normal.
	 */
	
	public BoardFocusMode2 (int files, int columnes, int pairs, int temps,int isGuest,ServerConnection sConnect,Client client,ButtonController controler, int level) {
		...
		for (int j = 0; j < this.getCards().size(); j++){
			...
			case 0:
				try{
					...
					this.getCards().get(j).setIcon(new ImageIcon(i));
